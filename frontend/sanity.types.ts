...
export type MediaLibraryReference = {
  ...
  _type: 'reference'
  _weak?: boolean
  [internalGroqTypeReferenceTo]?: 'mediaLibrary'
}
...
// Query: *[_type == 'page' && slug.current == $slug][0]{    _id,    _type,    name,    slug,    heading,    subheading,    parenthetical,    content[]{      ...,      markDefs[]{        ...,          _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }      }    },    "pageBuilder": pageBuilder[]{      ...,      _type == "callToAction" => {        ...,        button {          ...,            link {      ...,        _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }      }        }      },      _type == "infoSection" => {        content[]{          ...,          markDefs[]{            ...,              _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }          }        }      },    },  }
...
// Query: {    "previous": coalesce(      *[_type == "work" && defined(slug.current) && date < $currentDate] | order(date desc) [0] {        _id,        "title": coalesce(title, "Untitled"),        subtitle,        "slug": slug.current,        coverImage      },      *[_type == "work" && defined(slug.current) && _id != $currentId] | order(date desc) [0] {        _id,        "title": coalesce(title, "Untitled"),        subtitle,        "slug": slug.current,        coverImage      }    ),    "next": coalesce(      *[_type == "work" && defined(slug.current) && date > $currentDate] | order(date asc) [0] {        _id,        "title": coalesce(title, "Untitled"),        subtitle,        "slug": slug.current,        coverImage      },      *[_type == "work" && defined(slug.current) && _id != $currentId] | order(date asc) [0] {        _id,        "title": coalesce(title, "Untitled"),        subtitle,        "slug": slug.current,        coverImage      }    ),    "related": *[_type == "work" && defined(slug.current) && _id != $currentId && date != $currentDate] | order(date desc) [0] {      _id,      "title": coalesce(title, "Untitled"),      subtitle,      "slug": slug.current,      coverImage    }  }
